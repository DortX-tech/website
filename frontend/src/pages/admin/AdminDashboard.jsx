...
export default function AdminDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setSubscribers(subs.data.items || []);
    } catch (e) {
  ...
  const exportCsv = async () => {
    ...
    a.href = url; a.download = "dortx_leads.csv"; a.click();
    ...
    <div className="min-h-screen bg-[#05080F] noise" data-testid="admin-dashboard">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Users} label="Total leads" value={stats?.total_leads ?? "—"}/>
        ...
        <div className="mt-6 glass rounded-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-[13.5px]" data-testid="leads-table">
              ...
              <tbody className="divide-y divide-white/8">
                ...
                {leads.map((l) => (
                  <tr key={l.id} onClick={() => setSelected(l)} className="hover:bg-white/[0.03] cursor-pointer transition" data-testid={`lead-row-${l.id}`}>
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-[11px] px-2.5 py-1 rounded-full border ${STATUS_COLOR[l.status]}`}>{l.status}</span>
                    </td>
                    <td className="px-5 py-3 text-[#6B7385] hidden lg:table-cell">{new Date(l.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-white/8 text-[12.5px] text-[#9AA3B8]">
            <div>Showing {leads.length} of {total}</div>
            <div className="flex items-center gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-3 py-1 rounded-md border border-white/10 disabled:opacity-40">Prev</button>
              <span>Page {page}</span>
                ...
                <tbody className="divide-y divide-white/8">
                  ...
                  {applications.map((a) => (
                    <tr key={a.id} className="hover:bg-white/[0.03]">
                      ...
                      <td className="px-5 py-3 text-[#6B7385] hidden lg:table-cell">{new Date(a.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-white/8">
                  ...
                  {subscribers.map((s) => (
                    <tr key={s.id} className="hover:bg-white/[0.03]">
                      ...
                      <td className="px-5 py-3 text-[#6B7385] hidden lg:table-cell">{new Date(s.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {tab === "analytics" && (
          <div className="mt-6 grid lg:grid-cols-2 gap-4" data-testid="analytics-panel">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <div className="space-y-2.5">
                {(stats?.by_service || []).slice(0, 6).map((s) => (
                  <div key={s.service} className="flex justify-between text-[13px]">
                    <span className="text-[#C9D2E0]">{s.service}</span>
                    <span className="text-[#9AA3B8]">{s.count}</span>
                  </div>
                ))}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => setSelected(null)} className="fixed inset-0 z-50 bg-black/60 backdrop-blur flex items-end sm:items-center justify-center p-4">
          <motion.div initial={{ y: 30, scale: 0.98 }} animate={{ y: 0, scale: 1 }} onClick={(e) => e.stopPropagation()} className="glass-strong rounded-2xl max-w-2xl w-full p-7 max-h-[85vh] overflow-y-auto" data-testid="lead-detail-modal">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="font-display text-[22px] font-semibold text-white">{selected.name}</div>
                <a href={`mailto:${selected.email}`} className="text-[13.5px] text-[#4D8BFF]">{selected.email}</a>
              </div>
              <span className={`text-[11px] px-2.5 py-1 rounded-full border ${STATUS_COLOR[selected.status]}`}>{selected.status}</span>
            </div>
            <div className="grid sm:grid-cols-2 gap-4 mt-6 text-[13.5px]">
              ...
              <Info l="Created" v={new Date(selected.created_at).toLocaleString()}/>
            </div>
            <div className="mt-5">
